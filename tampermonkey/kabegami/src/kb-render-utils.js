(function (global) {
  ...
  utils.setVideoSource = utils.setVideoSource || function setVideoSource(video, src, fallbackSrc) {
    if (!video) return;

    const fallback = (fallbackSrc && fallbackSrc !== src) ? fallbackSrc : null;

    if (video.__kbVideoErrorHandler) {
      video.removeEventListener('error', video.__kbVideoErrorHandler);
    }
    ...
    video.__kbFallbackSrc = fallback;
    video.__kbTriedFallback = false;
    video.__kbVideoErrorHandler = function handleVideoError() {
      if (!video.__kbFallbackSrc || video.__kbTriedFallback) return;
      video.__kbTriedFallback = true;
      applySource(video.__kbFallbackSrc);
    };
    video.addEventListener('error', video.__kbVideoErrorHandler);

    if (video.dataset.src === src && src) {
      try {
